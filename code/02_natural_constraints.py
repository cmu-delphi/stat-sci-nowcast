...
def run_natural_constraints(as_of,
                            kernel_dict,
                            training_functions,
                            convolved_response_prefix,
                            convolved_truth_indicator,
                            overwrite = False):
    """Perform a single run of the TF vs. NTF experiment for the as_of date."""
    ...
    if not overwrite and os.path.isfile(os.path.join(Config.OUTPUT_DATA_PATH,
                                                     f'natural_constraints/as_of_{as_of}.p')):
    ...
    for fit_name, fit_func in training_functions.items():
        ...
        for i, (loc, data) in enumerate(convolved_ground_truth.items()):
            try:
                metadata_file = f'./results/natural_constraints/metadata/{fit_name}_{data.geo_value}_{as_of}.data'
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.DEBUG,
                        filename=f'logs/02_natural_constraints.log',
                        filemode="a+",
                        format="%(asctime)-15s %(levelname)-8s %(message)s")

    # Use standard trend filtering, with no additional smoothing.
    tf = partial(deconvolution.deconvolve_tf_cv, k=3,
                 fit_func=partial(deconvolution.deconvolve_tf, natural=False),
                 lam_cv_grid=np.r_[
                     np.logspace(1, 3.5, 10), [5000, 8000, 15000]],
                 gam_cv_grid=np.array([0]),
                 verbose=False)
    ntf = partial(deconvolution.deconvolve_tf_cv, k=3,
    ...
    training_functions = {'tf': tf, 'ntf': ntf}
    ...
    for as_of in as_of_date_range:
        ...
        pool_results.append(
            pool.apply_async(run_natural_constraints, args=(as_of,
                                                            kernel_dict,
                                                            training_functions,
                                                            convolved_response_prefix,
                                                            convolved_truth_indicator,
